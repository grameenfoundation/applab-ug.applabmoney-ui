...
<html>
<head>
<title>Transaction Settings Manager</title>
<link type="text/css"
...
<body>
	...
	<div>
		<div>
			<table style="width: 100%;">
				<tr>
					<td><h2>Transaction Receive Limits</h2>
						<hr>
					</td>
				</tr>
			</table>
						...
						<tr>
							...
							<td><div>
									<input class="btn" type="submit" name="submit"
										value="Create New"
										onClick="location.href='addglobalreceivelimit.html'">
								</div></td>
						</tr>
					</table> <c:if test="${empty globalReceiveLimitList}">
						<div>
							<table>
								<tr>
									<td>There are no Global Receive Limits configured on this
										system <br> <br></td>
								</tr>
							</table>
						</div>
					</c:if> <c:if test="${!empty globalReceiveLimitList}">
						<h2>System Global Recieve Limits</h2>
						<table class="data">
							<tr>
								<th>No</th>
								<th>Associated Account Type</th>
								<th>Associated Transaction Keyword</th>
								<th>Maximum Transaction Amount</th>
								<th>Minimum Transaction Amount</th>
								<th>Maximum Transaction Amount Per Day</th>
								<th>Maximum Transaction Amount Per Month</th>
								<th>Maximum Transactions Per Day</th>
								<th>Maximum Transactions Per Month</th>
								<th>Action</th>
							</tr>
							<c:forEach items="${globalReceiveLimitList}" var="receivelimit"
								varStatus="status">
								<tr class=data>
									<td class="data"><a
									...
									<td class=data><c:forEach items="${accountTypesList}"
											var="accountType">
											<c:if
												test="${accountType.accountTypeId == receivelimit.accountType.accountTypeId}">
												<c:if test="${accountType.accountTypeId != 0}">
													<c:out value="${accountType.accountTypeName}" />
												</c:if>
											</c:if>
										</c:forEach></td>
									<td class=data><c:forEach
											...
											<c:if
												test="${transkeyword.keywordId == receivelimit.transactionKeyword.keywordId}">
												<c:if test="${transkeyword.keywordId != 0}">
													<c:out value="${transkeyword.keywordName}" />
												</c:if>
											</c:if>
										</c:forEach></td>
									<td class=data>${receivelimit.minTransactionAmount}</td>
									<td class=data>${receivelimit.maxTransactionAmount}</td>
									<td class=data>${receivelimit.maxAmountDay}</td>
									<td class=data>${receivelimit.maxAmountMonth}</td>
