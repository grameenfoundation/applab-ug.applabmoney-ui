						...
						<tr>
							...
							<td><div>
									<input class="btn" type="submit" name="submit"
										value="Create New"
										onClick="location.href='addamlbarring.html'">
								</div>
							</td>
						</tr>
					</table> <c:if test="${empty amlBarringSettingsList}">
						<div>
							<table>
								<tr>
									<td>There are no AML barring settings configured for this
										system <br> <br>
									</td>
								</tr>
							</table>
						</div>
					</c:if> <c:if test="${!empty amlBarringSettingsList}">
						<h2>Configured AML Barring Settings Listing</h2>
						<table class="data">
							<tr>
								<th>AML Barring ID</th>
								<th>Account Type Assoc to AML Barring</th>
								<th>Keyword Assoc to AML Barring</th>
								<th>Allow Send</th>
								<th>Allow Receive</th>
								<th>Action</th>
							</tr>
							<c:forEach items="${amlBarringSettingsList}" var="amlbarring">
								<tr class=data>
									<td class=data><a
										href="/applabmoneyaui/editamlbarring/${amlbarring.barringId}.html">${amlbarring.barringId}</a>
									</td>
									<td class=data><c:forEach items="${accountTypesList}"
											var="accountType">
											<c:if
												test="${accountType.accountTypeId == amlbarring.accountType.accountTypeId}">
									...
									<td class=data><c:forEach
											items="${transactionKeywordsList}" var="keyword">
											<c:if
												test="${keyword.keywordId == amlbarring.transactionKeyword.keywordId}">
									...
									<td class=data><c:if test="${amlbarring.allowSend != 1}">
											<c:out value="No"></c:out>
										</c:if> <c:if test="${amlbarring.allowSend == 1}">
											<c:out value="Yes"></c:out>
										</c:if>
									</td>
									<td class=data><c:if
											test="${amlbarring.allowReceive != 1}">
											<c:out value="No"></c:out>
										</c:if> <c:if test="${amlbarring.allowReceive == 1}">
											<c:out value="Yes"></c:out>
										</c:if>
									</td>
									<td class=data><a
										href="/applabmoneyaui/removeamlbarring/${amlbarring.barringId}.html">
											Delete </a>
									</td>
								</tr>
							</c:forEach>
